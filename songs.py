...
MAIN_FOLDER = r"C:\Users\ksuma\Desktop\project x\songs"
...
if mood not in FOLDERS:
    ...
    if not songs:
        ...
        # Step 6: Function to listen for stop input ('s')
        def wait_for_stop():
            while pygame.mixer.music.get_busy():
                user_input = input("Press 's' to stop the song early: ").strip().lower()
                if user_input == 's':
                    pygame.mixer.music.stop()
                    print("⏹️  Song stopped manually.")
                    break
